        ...
        for e in msp:
            ...
            if e.dxf.layer == 'PKG_OUTLINE':
                if e.dxftype() == 'LWPOLYLINE':
                    ...
                    self.pkgout = spg.box(min(p)[0]*self.scaler,min(p)[1]*self.scaler,
                                          max(p)[0]*self.scaler,max(p)[1]*self.scaler)
...
def processjigstreetraw(fn, cmargin=0.65):
    ...
    rawdxf = loaddxf(dxffile,scaledrawing=.001)
    rawdxf.process()
...
# %%
# derp = processjigstreet(r"E:\Projects\MACD-1\TCB\Examples\raw\M47331-001_BSR_r01-Design Overlay.dxf")
# %%
